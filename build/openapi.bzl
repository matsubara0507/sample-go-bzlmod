...
def _openapi_generate_gen_impl(ctx):
    ...
    out_file_name = ctx.label.name + ".bash"
    bazel_root = "bazel-sample-go-bzlmod"
    arguments = [
        "-jar", paths.join(bazel_root, openapi_generator.path),
        "generate",
...
{cmd} {args}
        """.format(cmd = paths.join(bazel_root, java_home, "bin/java"), args = " ".join(arguments)),
        is_executable = True,
    )
